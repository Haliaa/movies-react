...
const getAllMovies = createAsyncThunk("getAllMovies", async ({ page }) => {
  const { data } = await movieService.getAllMovies(page);
  return data;
});

const getMovie = createAsyncThunk("getMovieById", async ({ id }) => {
...
const searchMovie = createAsyncThunk(
  "searchMovie",
  async ({ search, page }) => {
    const { data } = await movieService.searchMovieByTitle(search, page);
    return data;
  }
);
...
const {
  ...
  actions: { setMoviePage, setMovieSearch },
} = movieSlice;
const {
...
export { movieReducer, movieActions, themeReducer, themeActions };
